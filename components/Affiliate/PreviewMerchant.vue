    ...
    <div>
        <div v-if="merchant" class="p-8">
          <header class="flex flex-row justify-between pt-2">
            ...
            <button class="border rounded font-bold text-sm">Apply</button>
            </div>
          </header>
          <div>
          </div>
    ...
    methods:{
        ...
        refresh(){
          console.log(this.$store.state);
            ...
            this.$axios.get(route('api.merchants.show', this.merchantId))
                .then(response => {
        ...
        subscribe(){
            ...
            if(this.merchant.subscription_status === 'none'){
                if(!this.merchant.auto_approve_affiliates && !this.showApplicationTab){
                 ...
                 this.inFlight = true;
                 this.$axios.post(route('merchant.subscribe', this.merchantId),{  application: this.applicationText})
                .then(response => {
                    this.refresh();
                });
            }
